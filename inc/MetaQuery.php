 ...
 *
 * @copyright  This class base on code from WordPress Meta Query class (WP_Meta_Query).
 *
 ...
 *
 * MetaQuery is a helper that allows primary query classes, such as WP_Query and WP_User_Query,
 *
...
class MetaQuery {
	 ...
	 *                                            Array of meta query clauses. When first-order clauses or sub-clauses
	 *                                            use strings as their array keys, they may be referenced in the
	 *                                            'orderby' parameter of the parent query.
	 *
	 ...
	 *                                            which enables the `$key` to be cast to a new data type for
	 *                                            comparisons.
	 *
	 * @since       3.2.0
	 */
	public function __construct( $meta_query = false, $helpers = false ) {
		if ( $helpers )
	...
	public function get_sql( $type, $primary_table, $primary_id_column, $context = null ) {
		$meta_table = $this->Helpers->getMetaTableName( $type );
		if ( ! $meta_table )
			return false;
		 ...
		 *
		 */
		return apply_filters_ref_array( 'get_meta_sql_wpmo', array(
			$sql,
			$this->queries,
			$type,
			$primary_table,
	...
	public function get_sql_for_clause( &$clause, $parent_query, $clause_key = '' ) {
		...
		if ( ! $i && false === $alias ) {
			...
			if ( 'NOT EXISTS' === $meta_compare ) {
				...
				// } else {
				// $join .= $wpdb->prepare(" ON ( $this->primary_table.$this->primary_id_column = $alias.$this->meta_id_column AND $alias.%s IS NOT NULL )", $clause['key']);
				// TESTED: Done.
				$join .= " ON ( $this->primary_table.$this->primary_id_column = $alias.$this->meta_id_column AND $alias.{$clause['key']} IS NOT NULL )";
				// }
		...
		if ( array_key_exists( 'key', $clause ) ) {
			if ( 'NOT EXISTS' === $meta_compare ) {
				 ...
				 * nested clause.
				 */
				if ( in_array( $meta_compare_key, array(
					'!=',
					'NOT IN',
					'NOT LIKE',
					'NOT EXISTS',
	...
	protected function find_compatible_table_alias( $clause, $parent_query ) {
		 ...
		 * @param MetaQuery    $query        MetaQuery object.
		 *
		 * @since 4.1.0
		 *
		 */
