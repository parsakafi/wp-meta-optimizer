...
class Queries extends Base
{
    ...
    {
        $this->metaQuery = $metaQuery->meta_query = new MetaQuery();
        $metaQuery->meta_query->parse_query_vars($metaQuery->query_vars);
        $this->queryVars = $metaQuery->query_vars;
    }

    function changeTermsClauses($clauses, $taxonomies, $args)
    {
        $this->queryVars['order'] = isset($this->queryVars['order']) ? strtoupper($this->queryVars['order']) : '';
        $order       = $this->parse_order($this->queryVars['order']);
        // 'term_order' is a legal sort order only when joining the relationship table.
        $_orderby = $this->queryVars['orderby'];

        if (empty($this->queryVars['orderby']) || 'term_order' === $_orderby && empty($this->queryVars['object_ids'])) {
            $ordersby = ['term_id'];
    ...
    protected function termParseOrderbyMeta($orderby_raw)
    {
        ...
        $this->metaQuery->get_sql('term', 't', 'term_id');
        $meta_clauses = $this->metaQuery->get_clauses();
